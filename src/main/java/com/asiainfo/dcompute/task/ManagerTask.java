...
public class ManagerTask implements Runnable {
    ...
    public void run() {
        ...
        //获取redis调度分布式锁
        String lockId = this.manager.acquireManagerLock();
        if (StringUtils.isEmpty(lockId)) {
            LOGGER.info("服务器({})未能获取到分布式任务调度锁，已有其他服务器正在执行调度，直接返回！", this.serverId);
            return;
        }
        ...
        try {
            ...
            //带执行任务（剔除执行中的任务）
            List<Task> tasks = this.task.getExecuteTask();
            ...
            //任务分配写入
            this.manager.schedule(scheduleMap);
            LOGGER.info("服务器({})任务调度完成, 任务调度结果:{}", this.serverId, scheduleMap);
        } catch (Exception ex) {
            LOGGER.error("服务器({})执行任务调度时出现异常！\n{}", this.serverId, ex);
        } finally {
            LOGGER.info("服务器({})释放分布式任务调度锁(lockId={}){}！", this.serverId, lockId, 
                    this.manager.releaseManagerLock(lockId) ? "成功" : "失败");
        }
    }
}
